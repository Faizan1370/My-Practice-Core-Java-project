...
public class SottMM {
	...
	public static void sortMap() {
		getMapValues().entrySet().stream().sorted(Map.Entry.comparingByValue(Comparator.reverseOrder())).forEach(entry->System.out.println(entry));
	}
	
	public static void sortMap1() {
		getMapValues().entrySet().stream().sorted(Map.Entry.comparingByKey(Comparator.comparingInt(emp->((Employee) emp).getSalary()).reversed())).forEach(entry->System.out.println(entry));
	}
