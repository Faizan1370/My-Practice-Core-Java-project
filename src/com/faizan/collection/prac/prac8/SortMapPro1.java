...
public class SortMapPro1 {
	...
	public static void sortMap2() {
		getMapValues().entrySet().stream().sorted(Map.Entry.comparingByValue(Comparator.reverseOrder())).forEach(emp->System.out.println(emp));
	}
	public static void sortMap3() {
		getMapValues().entrySet().stream().sorted(Map.Entry.comparingByKey(Comparator.comparingInt(emp->((Employee)emp).getSalary()))).forEach(emp->System.out.println(emp));
	}
