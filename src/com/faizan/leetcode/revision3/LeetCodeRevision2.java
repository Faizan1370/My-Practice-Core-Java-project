...
public class LeetCodeRevision2 {
	...
	public static int[] indicesDiff(int[] nums, int valueDiff, int indexDiff) {
		...
		for (int i = 0; i < nums.length; i++) {
			...
			int target1 = nums[i] + valueDiff;
			int target2 = nums[i] - valueDiff;

			if (map.containsKey(target1) && Math.abs(i - map.get(target1)) >= indexDiff) {

				return new int[] { map.get(target1), i };
			}

			if (map.containsKey(target2) && Math.abs(i - map.get(target2)) >= indexDiff) {
