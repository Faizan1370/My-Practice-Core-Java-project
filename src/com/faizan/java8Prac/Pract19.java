...
public class Pract19 {
	...
	public static void sortElement() {
	 ...
	 public static void convertIntoMap() {
		 ...
		 list.stream().collect(Collectors.toMap(value->value, value->value.toUpperCase()))
		 .entrySet()
		 .stream()
		 .forEach(entry->System.out.println(entry));
	 }
	 ...
	 public static void kthHiehestSal() {
		 int k=2;
		 Integer integer = list.stream().sorted(Comparator.comparingInt(Employee::getSalary).reversed()).map(emp->emp.getSalary()).skip(k-1).findFirst().get();
		System.out.println(integer);
	 ...
	 public static void findSumReduce() {
		 ...
		 System.out.println(collect +" "+collect2 +" "+reduce +" "+sum);
	 ...
	 public static void optional() {
		 ...
		 list.stream().forEach(emp->{
			// System.out.println(Optional.of(emp.getName()));
			 ...
			 System.out.println(Optional.ofNullable(emp.getName()).map(name->name.toUpperCase()).orElse("Faizan"));
		 });
