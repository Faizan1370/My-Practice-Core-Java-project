...
public class Pract20 {
	...
	public static void sortElement() {
	 ...
	 public static void groupByDepWithHieghestSal() {
		 list.stream()
		 .collect(Collectors.groupingBy(emp->emp.getDesignation(),Collectors.maxBy(Comparator.comparingInt(emp->emp.getSalary()))))
		 .entrySet()
		 .stream()
		 .forEach(ent->System.out.println(ent));
	 }
	 ...
	 public static void completableFutureSuppltyAsnc() {
		 ExecutorService executorService = Executors.newFixedThreadPool(1);
	 ...
	 public static void findSumReduce() {
		 ...
		 Optional<String> reduce2 = words.stream().reduce((word1,word2)->word1.length()>word2.length()?word1:word2);
